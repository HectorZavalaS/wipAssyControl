...
namespace stocktake_V4
{
    public partial class Form1 : MetroForm
    {
        ...
        private void printLabel()
        {
            try
            {
                if (!String.IsNullOrEmpty(txtScanedSerial.Text)){ ////txtQRLocation.Text
                    if (!String.IsNullOrEmpty(txtQRMag.Text)){
                        if (!String.IsNullOrEmpty(txtQty.Text)){
                            if (!String.IsNullOrEmpty(txtQRLocation.Text)){ /// txtScanedSerial.Text
                                ...
                                if (zone != "NOT_FOUND")
                                {
                                    ...
                                    int response = 0;
                                    String mag = txtQRLocation.Text + "-" + txtQRMag.Text;
                                    ...
                                    {
                                        response = m_db_st.insertLabel_V2(idU, serial, model, djGroup, semifinish, cgsRoute, qty, qr, mag, type_cont, m_quarantine == true ? 1 : 0, m_id_area, txtQRMag.Text, txtQRLocation.Text,txtReview.Text).First().RESULT;
                                        if (response > 0)
                                        {
                                            m_db_st.updateLastQtyByUser(idU, count);

                                            if(havePrintLabel) m_print.sendToPrinter(namePrinter, template, ref error, mag);
                                            //m_print.sendToPrinterIP("192.168.6.30", template, ref error);
                                            m_console.AppendText("[" + DateTime.Now.ToString() + "] " + "Se guardo el registro.\n");
                                            getLastFive();
                                        ...
                                        else
                                        {
                                            ...
                                            {
                                                m_alerts.playWarning();
                                                m_dlgMessage.Message = "El magazine ya esta asignado a otra PCB.\n No se ingresa magazine.";
                                                m_dlgMessage.ShowDialog();
                                                CleanFields();
                                            }
                                        }
        ...
        private void chkBxPLabel_CheckedChanged(object sender, EventArgs e)
        {
            havePrintLabel = !havePrintLabel;
        }
    }
}
